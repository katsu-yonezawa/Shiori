...
デスクトップアプリでは、日常利用のログイン方式としてメール・パスワードを使い、補助的な方式として Magic Link を残します。OAuth はブラウザ遷移と deep link の扱いが増えるため、最初は後回しにします。

Supabase 側では、Authentication の Email provider を有効にします。メール・パスワードログインには `Email` と `Password` を使い、Magic Link には従来どおり OTP メール送信を使います。開発中に Magic Link の送信制限へ達した場合でも、既にパスワードを設定済みのユーザーはメール送信なしでログインできます。

新規ユーザーはアプリの「登録」ボタンから `signUp` できます。Supabase プロジェクトで Confirm email を有効にしている場合は、登録後に確認メールの承認が必要です。Confirm email を無効にしている場合は、登録直後にセッションが作成される設定になるため、運用方針に合わせて選択します。

既存の Magic Link ユーザーにパスワードを設定する場合は、Supabase Dashboard からユーザーのパスワードを設定するか、別途パスワードリセット導線を用意します。このチケットでは日常ログイン手段の追加を優先し、リセットメール送信のUIは後続で扱います。
...
- メール・パスワードログイン時に Magic Link メール送信が発生しないこと
- Magic Link の送信制限時に、メール・パスワードログインへ切り替えられること
